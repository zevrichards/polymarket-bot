...
## SESSION 21 -- TWAP resolution mechanism discovered; model corrected (2026-09-18)

### Goal
Diagnose an 11-trade losing streak (Aug 17 - Sep 9, all losses, ~$29 lost of $54.67), then explore two new strategy ideas the user proposed.

### The Aug 17-Sep 9 losses

Every losing trade followed the same shape: market priced a token at 0.5-3.5c (implying 97-99.5% confidence on the other side), our model (Session 20's loosened min_model_p=0.35, no price range) disagreed and bought the extreme longshot on a large apparent "edge." All 11 lost. Root cause traced to a historical backtest (scripts/backtest_lag_edge.py, 300 real resolved BTC markets via Gamma /events + Polymarket CLOB /prices-history + Binance klines) confirming: tokens priced under 5c have an actual win rate of 0% (n=144) -- worse than their own 2.5% implied odds. This is a confirmed trap, not noise.

### Two new strategies tested, both negative

User proposed (1) small bets against ~100:1 odds early in the window when price is still near baseline ("lottery"), and (2) betting with the market's early lean, confirmed by a 15-minute BTC trend ("trend-confirm"). Built scripts/backtest_new_strategies.py to test both:

- Lottery: the precondition never occurs. Across 1677 early-window checkpoints (15-120s into a 5-min window), the most extreme market price ever seen was 3.5% -- nowhere near 100:1. Market makers don't quote silly-extreme odds while 4+ minutes of real uncertainty remains; the mispricing this strategy needs doesn't exist in the data.
- Trend-confirm: betting with the market's early favorite lost money even at a 59.5% win rate (you're paying a premium for the favorite, so win rate alone isn't the bar). The 15-minute trend filter added nothing -- trend-confirmed subsets performed the same or slightly worse than the unconditional baseline at every margin tested.

### The real discovery: these markets resolve on TWAP, not spot price

While pulling a market's full description (/events?slug=), found the actual resolution rule: "This market will resolve to 'Up' if the time-weighted average price (TWAP) of Bitcoin, generated by Chainlink, of the time range specified in the title is greater than or equal to the price at the beginning of that range."

Every model in this repo (and every backtest run before this session) compared terminal spot price to baseline -- a different question than what actually settles the market (average price over the whole window vs baseline). This plausibly explains the entire pattern of "the market looks smarter than our model" observed since Session 19: a market pricing off the true TWAP mechanism will look more confident, earlier, than a model comparing single points ever would.

### Math: prob_up_twap (core/probability.py)

Split the window into REALIZED (window start to now -- already observed, zero variance) and REMAINING (now to window end -- modeled the same driftless way as prob_up, E[future price] = current_price). The window's final TWAP is the time-weighted blend of the two. Key result borrowed from Asian-option pricing theory: the variance of the time-average of a Brownian path over duration tau is sigma^2 * tau / 3 -- one third the variance of the tau-ahead endpoint alone (sigma^2 * tau). An average is anchored by near-term values close to the current level as well as the more-wandered-off far values, so it's inherently less noisy than the raw endpoint.

Deliberate simplification: works in price space (Normal), not log space (lognormal) like prob_up -- summing lognormal path segments has no closed form, summing Normal ones does, and for a 5-minute BTC window the relative price range is small enough that this is a negligible difference (same category of short-horizon simplification as the Ito correction prob_up already omits, see Session 9).

Counterintuitive but verified result (see tests/test_probability.py): early in the window, for the same displacement/vol/time, prob_up_twap returns a more extreme probability than prob_up -- not less. The averaging target has lower variance, so the same displacement is more statistically significant against it.

baseline_price is still approximated as spot-at-window-start (not a true opening TWAP reading) -- a known, smaller simplification than the realized-vs-terminal distinction this session actually fixes.

### Backtest re-run with the corrected model -- the old edge was mostly a model-mismatch artifact

Re-ran backtest_lag_edge.py against the same 300 cached markets, old model vs new:

```
                          old (spot)                  new (TWAP-aware)
current live config:      93 trades, +$41.18          101 trades, -$33.73   <- flips hard negative
old tight (.75/[.3,.7]):  17 trades, +$14.83           22 trades,  -$0.74   <- flips to ~breakeven
old wide (.75/[.15,.85]): 37 trades, +$21.59           47 trades,  +$1.00   <- edge nearly gone
moderate (.55/[.1,.9]):   51 trades, +$31.95           64 trades, +$14.44   <- edge cut by more than half
no model_p floor:        117 trades, -$22.39          102 trades, -$35.73   <- still bad either way
```

The corrected model's verdict on "current live config" (-$33.73) now agrees with what actually happened live (the 11-loss streak) -- the old backtest's +$41.18 for that exact config was a mirage caused by comparing the wrong quantity. This is a strong signal the TWAP fix addresses a real problem, not a spurious one: a model that only agrees with reality after being fixed is a lot more trustworthy than one that only agreed with itself.

"Moderate" (min_model_p=0.55, entry_price_range=[0.10,0.90]) remains the best performer, but its edge roughly halved once measured correctly (+$14.44 vs the old +$31.95 over a similar trade count). Real, but smaller than believed.

### Changes applied

1. core/probability.py: added prob_up_twap() alongside the existing prob_up() (kept, not removed -- still correct for anything that genuinely resolves on a terminal/spot comparison). 8 new tests in tests/test_probability.py, including a direct check of the "more confident, not less" derivation.
2. core/binance_client.py: added get_prices_between() to fetch the realized price path for a window (one extra API call per tick per tracked market -- negligible given we track ~0-1 markets at a time in the final-30s window).
3. bots/lag_bot.py: compute_signal() now fetches the realized window path and calls prob_up_twap instead of prob_up. Signal dict gains realized_avg_price.
4. scripts/backtest_lag_edge.py: build_dataset() updated to compute realized_avg and use prob_up_twap. Also refactored the market-fetch caching (load_or_fetch_markets, 6h TTL to logs/resolved_btc_markets_cache.json) out of main() so it's shared with backtest_new_strategies.py -- apples-to-apples market sets across backtest runs, and no more re-paying the several-minute Gamma slug-walk every run.
5. scripts/backtest_new_strategies.py: lottery collection also switched to prob_up_twap.
6. Test isolation bug fixed: test_check_stop_losses_exits_position_beyond_threshold didn't monkeypatch core.journal's write path, so every full test-suite run wrote a fake stop_loss_exit record (market_id="m1") straight into the real production logs/trades.jsonl. Found two such entries after running the suite twice this session; removed them and fixed the test to isolate its journal writes like test_count_todays_entries already does.

### Status
- Paper-mode validation (mode="paper", "moderate" filters) restarted with the corrected TWAP-aware code -- it had been running for hours on the old spot-based model before this fix, so it was restarted rather than left to accumulate meaningless results. Zero real trades occurred before the restart (confirmed via trade journal), so nothing was lost.
- All 119 tests pass, journal write isolated and verified clean.
