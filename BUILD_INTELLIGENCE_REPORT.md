...
**The bots cannot be assumed to survive indefinitely on this machine as currently launched -- always verify with `Get-CimInstance Win32_Process` before trusting that a long gap between checks means the bots kept running.** If sleep/power management keeps killing background processes, consider: disabling sleep while testing, or using a launch method less prone to being killed on suspend (Windows Task Scheduler with "wake the computer to run this task," or simply accepting that multi-hour unattended runs need the machine kept awake).
